...
$(function() {
  function MyCustomViewModel (parameters) {
    ...
    self.getAdditionalControls = function() {
      var settings = self.settings.settings.settings.plugins.E3V2_Leveling
      ...
      var baseSettings = [{
        'customClass': '', 'layout': 'horizontal_grid', 'name': 'Bed Leveling', 'children':
          ...
          {'width': '2', 'commands':
            [
            'G90', 'G0 Z' + settings.upper_z() + ' F500',
            'G0 X' + settings.front_left_x() + ' Y' + settings.front_left_y() + ' F' +
            settings.feed_rate(), 'G0 Z' + settings.lower_z() + ' F500',
            'G4 P' + settings.wait_for_tweak(), 'M300 P100', 'G4 P' + settings.wait_after_tweak(),
            'G90', 'G0 Z' + settings.upper_z() + ' F500',
            'G0 X' + settings.front_right_x() + ' Y' + settings.front_right_y() + ' F' +
            settings.feed_rate(), 'G0 Z' + settings.lower_z() + ' F500',
            'G4 P' + settings.wait_for_tweak(), 'M300 P100', 'G4 P' + settings.wait_after_tweak(),
            'G90', 'G0 Z' + settings.upper_z() + ' F500',
            'G0 X' + settings.back_right_x() + ' Y' + settings.back_right_y() + ' F' +
            settings.feed_rate(), 'G0 Z' + settings.lower_z() + ' F500',
            'G4 P' + settings.wait_for_tweak(), 'M300 P100', 'G4 P' + settings.wait_after_tweak(),
            'G90', 'G0 Z' + settings.upper_z() + ' F500',
            'G0 X' + settings.back_left_x() + ' Y' + settings.back_left_y() + ' F' +
            settings.feed_rate(), 'G0 Z' + settings.lower_z() + ' F500',
            'G4 P' + settings.wait_for_tweak(), 'M300 P100', 'G4 P' + settings.wait_after_tweak(),
            'G90', 'G0 Z' + settings.upper_z() + ' F500',
            'G0 X' + 0  + ' Y' + 0 + ' F' + settings.feed_rate(),
            'G90', 'G0 Z' + settings.final_z() + ' F500'
            ],
          'customClass': 'btn', 'additionalClasses': 'nowrap', 'name': 'Auto Run'},
          {'width': '7', 'commands':
            ['M0', 'G0 X' + 0  + ' Y' + 0 + ' F' + settings.feed_rate(),
            'G90', 'G0 Z' + settings.final_z() + ' F500',
            'M300 P200' ],
          'customClass': 'btn', 'additionalClasses': 'nowrap', 'name': 'Stop Auto', 'offset': '2'}]
      }];
